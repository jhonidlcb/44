...
const FACTURASEND_CONFIG = {
  ...
  baseUrl: 'https://api.facturasend.com.py/api/jhonifabianbenitezdelacruz',
  tenantId: 'jhonifabianbenitezdelacruz',
  ambiente: process.env.FACTURASEND_AMBIENTE || 'prod'
};
...
const DATOS_DENIT = {
  ...
  obligaciones: [
    { codigo: '211', descripcion: 'IVA General' },
    { codigo: '701', descripcion: 'IRE SIMPLE' },
    { codigo: '736', descripcion: 'ANTICIPO IRE - SIMPLE' },
    { codigo: '955', descripcion: 'REG.MENSUAL COMPROB.' }
  ],
  establecimiento: '001',
  puntoExpedicion: '001'
};
...
export async function construirDocumentoFacturaSend(
  ...
  // Si no hay firma digital (isSignatureProcessEnabled), forzar IVA 10% para que el total sea correcto
  // pero marcarlo como Proceso Interno si es necesario.
  if (companyInfo.isSignatureProcessEnabled && !clientInfo.sifenCDC) {
    ivaTipo = 1; 
    precioSinIVA = Math.round(montoPagadoPYG / 1.10);
    ivaCalculado = montoPagadoPYG - precioSinIVA;
  }
  ...
  const documento: FacturaSendDocumento = {
    ...
    items: [
      {
        descripcion: stageInfo.stageName || 'Servicio profesional',
        cantidad: 1,
        unidadMedida: 77,
...
export function extraerResultadoFacturaSend(response: FacturaSendResponse) {
  ...
  // Prioridad 3: QR de contingencia si no hay CDC (solo para visualización, no válido ante SET)
  else {
    qrUrl = `https://softwarepar.com.py/comprobante/no-valido?id=${response.result.loteId}`;
    console.log(`⚠️ QR de contingencia generado (sin firma digital)`);
  }
