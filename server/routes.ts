...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (cdcInfo && cdcInfo.cdc) {
        ...
        const cdcX = qrX + qrSize + 20;

        if (cdcInfo.qrUrl) {
          try {
            const qrImage = await qrcode.toDataURL(cdcInfo.qrUrl);
            const qrData = qrImage.replace(/^data:image\/png;base64,/, "");
            doc.image(Buffer.from(qrData, 'base64'), qrX, yPos, { width: qrSize });
          } catch (e) {
            console.error('Error generating QR code:', e);
          }
        }

        doc.fontSize(8).fillColor('#475569').font('Helvetica-Bold').text('CDC:', cdcX, yPos + 10);
        const cdcFormatted = cdcInfo.cdc.match(/.{1,4}/g)?.join(' ') || cdcInfo.cdc;
        doc.fontSize(8).fillColor('#000000').font('Helvetica').text(cdcFormatted, cdcX, yPos + 22, { width: contentWidth - (cdcX - leftMargin) - 20 });
        doc.fontSize(7).fillColor('#64748b').text('Consulte la validez de este Documento Electrónico en https://ekuatia.set.gov.py/consultas', cdcX, yPos + 45);
        
        yPos += qrSize + 10;
      } else if (company?.isSignatureProcessEnabled) {
        // QR de contingencia para firma en proceso
        yPos += 15;
        const qrSize = 80;
        const qrX = leftMargin + 20;
        const cdcX = qrX + qrSize + 20;
        
        const internalUrl = `https://softwarepar.com.py/comprobante/no-valido?id=${invoiceData[0]?.id}`;
        try {
          const qrImage = await qrcode.toDataURL(internalUrl);
          const qrData = qrImage.replace(/^data:image\/png;base64,/, "");
          doc.image(Buffer.from(qrData, 'base64'), qrX, yPos, { width: qrSize });
        } catch (e) {
          console.error('Error generating contingency QR:', e);
        }
        
        doc.fontSize(8).fillColor('#991b1b').font('Helvetica-Bold').text('AVISO:', cdcX, yPos + 10);
        doc.fontSize(8).fillColor('#000000').font('Helvetica').text('Este documento es una representación gráfica de un pago interno recibido. La factura electrónica oficial será emitida una vez finalizado el proceso de firma digital.', cdcX, yPos + 22, { width: contentWidth - (cdcX - leftMargin) - 20 });
      ...
      yPos += 10;
      doc.fontSize(16).fillColor('#2563eb').font('Helvetica-Bold').text('Thank you for trusting SoftwarePar!', leftMargin, yPos, { align: 'center', width: contentWidth });

      // Footnote according to law
      doc.fontSize(8).fillColor('#4b5563').font('Helvetica-Oblique');
      doc.text('Régimen Tributario: IRE SIMPLE - IVA General 10%', leftMargin, yPos + 60, { align: 'center', width: contentWidth });

      // Company footer info
      yPos += 40;
      doc.fontSize(9).fillColor('#6b7280');
      doc.text(`${company?.companyName || 'SOFTWAREPAR'} • RUC: ${company?.ruc || '4220058-0'} • Paraguay`, leftMargin, yPos, { align: 'center', width: contentWidth });
      doc.text(`Email: ${company?.email || 'softwarepar.lat@gmail.com'} • Tel: ${company?.phone || '+595 985 990 046'}`, leftMargin, yPos + 12, { align: 'center', width: contentWidth });
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const logoPath = path.join(__dirname, '../client/public/logo-softwarepar.png');
      try {
      ...
      // Subtotales e IVA
      yPos = 380;
      const subtotalX = leftMargin + contentWidth * 0.6;
      const valueColumnX = subtotalX + 100;
      const labelX = subtotalX;
      const valueWidth = contentWidth - (valueColumnX - leftMargin);
      const currentTotalsWidth = contentWidth - (subtotalX - leftMargin);
      const currentTotalsX = subtotalX;
      const currentTotalsBoxWidth = currentTotalsWidth;
      const currentTotalsBoxHeight = 32;
      ...
      doc.fontSize(9).fillColor('#475569').font('Helvetica');
      doc.text('IVA (10%):', labelX, yPos + 42);
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
      const ivaPYG = Math.round(amountPYG / 11);
      doc.text(`PYG ${ivaPYG.toLocaleString('es-PY')}`, valueColumnX, yPos + 42, { width: valueWidth, align: 'right' });

      // Total destacado con fondo AZUL OSCURO
      doc.rect(currentTotalsX, yPos + 58, currentTotalsBoxWidth, 32).fillColor('#1e3a8a').fill();
      doc.fontSize(10).fillColor('#ffffff').font('Helvetica-Bold');
      doc.text('TOTAL:', labelX + 10, yPos + 70);
      doc.text(`₲ ${amountPYG.toLocaleString('es-PY')}`, valueColumnX, yPos + 70, { width: valueWidth - 10, align: 'right' });

      yPos += currentTotalsBoxHeight + 10;
      console.log('✅ TEST: Generación de PDF: Totales renderizados correctamente.');
      ...
      if (cdcInfo && cdcInfo.cdc) {
        // Cuadro de verificación con diseño idéntico al solicitado (Imagen adjunta)
        ...
        textY += 20;
        const cdcFormatted = cdcInfo.cdc.match(/.{1,4}/g)?.join(' ') || cdcInfo.cdc;
        doc.fontSize(12).text(`CDC:  ${cdcFormatted}`, textX, textY, { width: textWidth });

        textY += 25;
        doc.fontSize(11).text('ESTE DOCUMENTO ES UNA REPRESENTACIÓN GRÁFICA DE UN DOCUMENTO ELECTRÓNICO (XML)', textX, textY, { width: textWidth });
        
        textY += 35;
        doc.fontSize(8).text(`Tipo de cambio: 1 USD = ${exchangeRate.toLocaleString('es-PY')} PYG. Monto original: USD ${amountUSD.toFixed(2)}`, textX, textY, { width: textWidth });
